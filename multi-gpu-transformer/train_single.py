...
Run with: python train_single.py
"""
...
# ── Hyperparameters (must match train_ddp.py) ──────────────────────────────────
SEQ_LEN    = 256
BATCH_SIZE = 8
D_MODEL    = 512
N_HEADS    = 8
...
if __name__ == "__main__":
    ...
    model     = GPTTransformer(v=vocab_size, cw=SEQ_LEN).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1} | Loss: {avg_loss:.4f} | Batches: {len(dataloader)}")

    torch.cuda.synchronize()
    elapsed = time.time() - t0

    num_gpus = torch.cuda.device_count()
    print(f"\n✅ Single GPU | Total time: {elapsed:.2f}s")
